...
function Beam({ fx }: { fx: ShotFx }) {
  ...
  // Jagged crackle offsets for the taser arc.
  const zig = useMemo(
    () => (fx.weapon === "taser" ? Array.from({ length: 4 }, () => (Math.random() - 0.5) * 0.5) : []),
    [fx]
  );
  ...
  useFrame(() => {
    ...
    g.scale.set(flick, 1, flick);
  });

  const glow = (c: string, r: number) => (
    <meshBasicMaterial color={c} transparent opacity={1} toneMapped={false} blending={THREE.AdditiveBlending} depthWrite={false} />
  );
